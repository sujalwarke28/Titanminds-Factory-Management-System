    ...
    <div className="animate-fade-in-up">
      <div style={{ marginBottom: 'var(--spacing-8)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)' }}>Manager Portal / Factory & Machine Live Status</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', background: isOnline ? 'rgba(34, 197, 94, 0.1)' : 'rgba(239, 68, 68, 0.1)', borderRadius: 'var(--radius-full)', color: isOnline ? 'var(--color-success)' : 'var(--color-danger)', border: `1px solid ${isOnline ? 'var(--color-success)' : 'var(--color-danger)'}` }}>
          {isOnline ? <Wifi size={18} /> : <WifiOff size={18} />}
          <span style={{ fontWeight: '600' }}>{isOnline ? 'Factory Online' : 'Factory Offline'}</span>
        </div>
      </div>

      <div style={{ marginBottom: 'var(--spacing-8)' }}>
        <h2 style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: 'var(--spacing-4)' }}>Critical Machine 3D Digital Twin (CNC_01)</h2>
        {!isOnline && (
          <div style={{ padding: 'var(--spacing-4)', background: 'rgba(239, 68, 68, 0.05)', border: '1px solid var(--color-danger)', borderRadius: 'var(--radius-md)', color: 'var(--color-danger)', marginBottom: 'var(--spacing-4)' }}>
      ...
      <h2 style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: 'var(--spacing-4)' }}>Factory Level Sensor Aggregations</h2>
      
      {!isOnline && (
        <div style={{ padding: 'var(--spacing-4)', background: 'var(--bg-surface)', border: '1px dashed var(--border-color)', borderRadius: 'var(--radius-md)', color: 'var(--text-muted)', marginBottom: 'var(--spacing-4)', textAlign: 'center' }}>
          No reading. Network disconnected.
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 'var(--spacing-6)' }}>
        {charts.map((c, i) => {
          ...
          const yAxisProps = c.isTemp ? getTempYAxisProps(streamData) : { tickFormatter: (v) => Number(v).toFixed(0) };
